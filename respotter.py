...
from scapy.layers.netbios import NBNSQueryRequest, NBNSQuery, NBNSQueryResponse
...
class Respotter:
        ...
        if response is not None and response.haslayer(DNS):
            ...
            for i in range(response[DNS].ancount):
                if response[DNS].an[i].type == 1:  # Type 1 is A record, which contains the IP address
                    print(response[DNS].an[i].rdata)  # rdata field of the A record contains the IP address
    ...
    def send_nbns_request(self):
        ...
        packet = IP(dst="255.255.255.255")/UDP(dport=137)/NBNSQueryRequest(QUESTION_NAME=self.hostname, QUESTION_TYPE=NBNSQuery())
        response = sr1(packet, verbose=0)  # Send the packet and capture the response
        if response is not None and response.haslayer(NBNSQueryResponse):
            ...
            for i in range(response[NBNSQueryResponse].ancount):
                if response[NBNSQueryResponse].an[i].TYPE == 1:
                    print(response[NBNSQueryResponse].an[i].rdata)
